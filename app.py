...
with col1:
    st.image("assets/new-e.png", width=300)

with col2:
    st.image("assets/winch4ai.png", width=500)

st.header("Application Démo / Mise en forme DC / New-e", divider="blue")
...
def generate(texte, fichiers):
    ...
    # Création de l'objet à passer à la fonction
    entry_model=[]
    for f in fichiers:
        entry_model.append(f)
    entry_model.append(texte)
...
"""
    ...
    with open("replaced.pdf","rb") as out_pdf_f:
        fichier_final = out_pdf_f.read()

    #fichier_final = "New-E-DC-prenom.pdf"
    return fichier_final

## ------------ Recevoir plusieur fichiers ----------

uploaded_file = st.file_uploader(
    "CV en format PDF", accept_multiple_files=True, type=['pdf', 'docx']
)
...
def callback_button(hidden) -> None:
    ...
    trigger = trigger_download(gen_pdf(notes_entretien, fichiers), "New-E-DC-prenom.pdf")
    # script is dumped at the end of the page
    with hidden:
        components.html(html=trigger, height=0, width=0)
    return

st.button('Dossier de compétences', on_click=callback_button, args=(hide_me,))
"""

def download_pdf():
    out_pdf = gen_pdf(notes_entretien, fichiers)
    st.download_button(label="Télécharger le dossier de compétences", data=out_pdf, file_name=f"New-E-DC-prenom.pdf", icon="📩")

st.button('Générer le dossier de compétences', on_click=download_pdf())
